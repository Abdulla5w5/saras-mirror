# Credits — Sara's Mirror

A game jam entry built for the theme **Illusion**.

## Art assets

- **Sara / player sprite** — "Free Schoolgirls Anime Character Pixel Sprite Pack" (Girl_2), CraftPix.net free license.
- **Enemies** — "Free Vampire Pixel Art Sprite Sheets" (Countess Vampire, Vampire Girl), CraftPix.net free license.
- **Illusion props / rock, plant, ruin sprites** — "Free Cursed Land Top-Down Pixel Art Tileset", CraftPix.net free license.
- **Trees** — "Free Tree Pixel Art Asset Pack", CraftPix.net free license.
- **Desert props** (palms, rocks) — "Sketch Desert" by Kenney (kenney.nl), CC0.
- **Gate + ground textures** (locked metal gate, sand, swamp water) — "Retro Textures: Fantasy" by Kenney (kenney.nl), CC0.
- **True Sight reveal FX** (sparkle) — "Ninja Adventure" pack by Pixel-boy / Sparklin Labs (superpowers-html5.com), CC0.

CraftPix free packs require a credit/backlink to https://craftpix.net when used — included here.

## Mechanics & art adapted from "Shards of Reflection"

The ornate museum-mirror art, the title-screen layout, and several obstacle
mechanics (crumbling floor, phase gate, sweeping light hazard, alignment-lever
puzzle) are adapted from the companion project *Shards of Reflection* — all
vector/code-drawn, no image assets.

## Code, design, audio, VFX

All gameplay code, level design, dialogue, procedural audio (synthesized at
runtime, no audio files), and procedural VFX/shaders were built for this jam.

## Engine

Godot Engine 4.6.

## Music

- **Music** — "Theater of Illusions (Unstopping Noise)" from Chaos Zero Nightmare OST by Artis4.
